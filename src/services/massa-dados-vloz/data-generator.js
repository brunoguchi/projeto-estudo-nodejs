...
const QTD_REGISTROS = 2000; // Quantidade de transações a gerar
const BATCH_SIZE = 1000;

// Arrays de IDs para geração aleatória
const CLIENTES_IDS = [200001];
const ADQUIRENTES_IDS = [
    '81B81F2F-5E8C-412C-AE21-0D588DE78E5F' //Stone
];

const PLANO_ID = 'EFD6324E-7433-4E2D-BB96-CFCC49E3B1BA';
const CAMPANHA_ID = 'E9840C0A-FF96-4928-800A-9CB0AABCB18D';
const USUARIO_ATUALIZACAO = 'usuario-automacao';
const DATA_INICIAL = new Date('2026-01-01T00:00:00Z');
...
function buildTransacaoTable(rows) {
    ...
    table.columns.add('PlanoId', sql.UniqueIdentifier, { nullable: true });
    table.columns.add('DataCaptura', sql.DateTime2, { nullable: false });
    table.columns.add('DataAutorizacao', sql.DateTime2, { nullable: false });
    table.columns.add('FormaPagamento', sql.NVarChar(sql.MAX), { nullable: false });
    ...
    table.columns.add('BandeiraCartao', sql.NVarChar(sql.MAX), { nullable: false });
    table.columns.add('StatusTransacao', sql.NVarChar(sql.MAX), { nullable: false });
    table.columns.add('ModoPagamentoCartao', sql.NVarChar(sql.MAX), { nullable: false });
    ...
    rows.forEach((r) => {
        table.rows.add(
            ...
            r.BandeiraCartao, r.StatusTransacao, r.ModoPagamentoCartao, r.IdentificadorUnico,
...
function buildParcelaTable(rows) {
    ...
    table.columns.add('Status', sql.NVarChar(sql.MAX), { nullable: false });
    table.columns.add('DataPagamento', sql.DateTime2, { nullable: true });
    table.columns.add('ValorAntecipacao', sql.Decimal(18, 2), { nullable: false });
    table.columns.add('ValorMdr', sql.Decimal(18, 2), { nullable: false });
    ...
    rows.forEach((r) => {
        table.rows.add(
            ...
            r.DataVencimento, r.Status, r.DataPagamento, r.ValorAntecipacao, r.ValorMdr,
            r.PercentualSplit, r.ValorSplit, r.DataUltimaAtualizacao, r.UsuarioUltimaAtualizacao, r.DataCriacao
        );
    });
    return table;
}

function buildApuracaoCampanhaTable(rows) {
    const table = new sql.Table('ApuracaoCampanha');
    table.create = false;
    table.columns.add('Id', sql.UniqueIdentifier, { nullable: false });
    table.columns.add('ClienteId', sql.BigInt, { nullable: false });
    table.columns.add('TransacaoId', sql.UniqueIdentifier, { nullable: false });
    table.columns.add('CampanhaId', sql.UniqueIdentifier, { nullable: false });
    table.columns.add('Competencia', sql.NVarChar(300), { nullable: false });
    table.columns.add('ValorCampanha', sql.Decimal(18, 2), { nullable: false });
    table.columns.add('DataUltimaAtualizacao', sql.DateTime2, { nullable: false });
    table.columns.add('UsuarioUltimaAtualizacao', sql.NVarChar(300), { nullable: false });
    table.columns.add('DataCriacao', sql.DateTime2, { nullable: false });
    ...
    rows.forEach((r) => {
        table.rows.add(
            r.Id, r.ClienteId, r.TransacaoId, r.CampanhaId, r.Competencia, r.ValorCampanha,
            r.DataUltimaAtualizacao, r.UsuarioUltimaAtualizacao, r.DataCriacao
        );
    });
    return table;
}

function buildContasPagarCampanhaTable(rows) {
    const table = new sql.Table('ContasPagarCampanha');
    table.create = false;
    table.columns.add('Id', sql.UniqueIdentifier, { nullable: false });
    table.columns.add('ClienteId', sql.BigInt, { nullable: false });
    table.columns.add('CampanhaId', sql.UniqueIdentifier, { nullable: false });
    table.columns.add('Competencia', sql.NVarChar(300), { nullable: false });
    table.columns.add('Valor', sql.Decimal(18, 2), { nullable: false });
    ...
    rows.forEach((r) => {
        table.rows.add(
            r.Id, r.ClienteId, r.CampanhaId, r.Competencia, r.Valor, r.Status,
            r.DataUltimaAtualizacao, r.UsuarioUltimaAtualizacao, r.DataCriacao
        );
    });
    return table;
    ...
    try {
        for (let i = 0; i < QTD_REGISTROS; i += BATCH_SIZE) {
            ...
            for (let j = 0; j < BATCH_SIZE && (i + j) < QTD_REGISTROS; j++) {
                ...
                const statusTransacao = String(randomInt(1, 4));
                const bandeiraCartao = String(randomInt(1, 3));
                const modoPagamento = String(randomInt(1, 3));
                const formaPagamento = String(randomInt(1, 4));
                ...
                // Gerar ApuracaoCampanha para esta transação
                const competencia = formatCompetencia(DataCaptura);
                const valorCampanha = randomFloat(1, 10);
                
                apuracoesCampanha.push({
                    Id: uuidv4(),
                    ClienteId: clienteId,
                ...
                for (let p = 1; p <= quantidadeParcelas; p++) {
                    ...
                    const valorAntecipacao = randomFloat(0, 2);
                    const valorMdr = randomFloat(0.5, 3);
                    ...
                    parcelas.push({
                        ...
                        DataPagamento,
                        ValorAntecipacao: parseFloat(valorAntecipacao.toFixed(2)),
                        ValorMdr: parseFloat(valorMdr.toFixed(2)),
            ...
            // Gerar ContasPagarCampanha agregado por competência e cliente
            const contasPagarMap = new Map();
            apuracoesCampanha.forEach((apuracao) => {
                const key = `${apuracao.ClienteId}_${apuracao.Competencia}`;
                if (!contasPagarMap.has(key)) {
                    contasPagarMap.set(key, {
                        ClienteId: apuracao.ClienteId,
                        Competencia: apuracao.Competencia,
                        Valor: 0,
                        DataCriacao: apuracao.DataCriacao,
                        DataUltimaAtualizacao: apuracao.DataUltimaAtualizacao
                    });
                }
                const conta = contasPagarMap.get(key);
                conta.Valor += apuracao.ValorCampanha;
            });

            const contasPagarCampanha = Array.from(contasPagarMap.values()).map((conta) => ({
                Id: uuidv4(),
                ClienteId: conta.ClienteId,
                CampanhaId: CAMPANHA_ID,
                Competencia: conta.Competencia,
                Valor: parseFloat(conta.Valor.toFixed(2)),
                Status: randomInt(0, 2) === 1,
                DataUltimaAtualizacao: conta.DataUltimaAtualizacao,
                UsuarioUltimaAtualizacao: USUARIO_ATUALIZACAO,
                DataCriacao: conta.DataCriacao
            }));

            console.log(`Batch: ${transacoes.length} transações, ${parcelas.length} parcelas, ${apuracoesCampanha.length} apurações, ${contasPagarCampanha.length} contas a pagar`);
            ...
            try {
                ...
                console.error('Erro ao inserir TransacaoParcela:', err);
                throw err;
            }

            try {
                await pool.request().bulk(buildApuracaoCampanhaTable(apuracoesCampanha));
            } catch (err) {
                console.error('Erro ao inserir ApuracaoCampanha:', err);
                throw err;
            }

            try {
                await pool.request().bulk(buildContasPagarCampanhaTable(contasPagarCampanha));
            } catch (err) {
                console.error('Erro ao inserir ContasPagarCampanha:', err);
                throw err;
            }
